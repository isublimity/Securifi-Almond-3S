...
### 1.3 Формат buf[5] (charging)

Из discharge лога: `buf[5] = 0x00` всегда (не заряжается).
Из charge лога (`Battery_Charge_logs1.txt`, 34 строки): `buf[5] = 0x01` при зарядке.

Текущий код `data_collector.c`:
```c
bi->charging = (raw[5] & 0x01) ? 1 : 0;
```
Проверка bit0 — корректна. Остальные биты buf[5] пока не исследованы.
