...
|--------|------|--------|------------|
| **24.10.x** | **6.6.127** | **РАБОТАЕТ** | Проверено, стабильно |
| 25.12.0 (main) | 6.12.71+ | **НЕ ПРОВЕРЕНО** | У нас не загрузилось (U-Boot от a43). По данным ar2r2806 — работает с U-Boot от DragonBluep |

**Рекомендация**: использовать ветку `almond-3s` (openwrt-24.10, ядро 6.6.x) как проверенную. OpenWrt 25.12 (ядро 6.12.x) требует проверки — возможно зависит от версии U-Boot.
