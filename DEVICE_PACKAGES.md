...
| Пакет | Зачем | Критичность |
|-------|-------|-------------|
| **kmod-i2c-core** | Ядро I2C подсистемы Linux | **ОБЯЗАТЕЛЕН** — lcd_drv.ko использует i2c_transfer() для PIC battery. MT7530 switch тоже требует I2C |
| **kmod-i2c-mt7621** | I2C контроллер MT7621 (palmbus SM0) | **ОБЯЗАТЕЛЕН** — физический I2C bus 0. Конфликт с тачскрином решён через SM0 save/restore в lcd_drv.ko. **НИКОГДА не выгружать/отключать** — ломает MT7530 LAN |
...
|--------|------|--------|------------|
| **24.10.x** | **6.12.74** | **РАБОТАЕТ** | Проверено, стабильно |

**Рекомендация**: использовать ветку `almond-3s` (openwrt-24.10, ядро 6.12.74).
